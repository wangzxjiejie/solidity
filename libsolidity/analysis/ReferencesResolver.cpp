...
bool ReferencesResolver::visit(InlineAssembly const& _inlineAssembly)
{
	...
	struct ResolveYul: public yul::ASTWalker
	{
		ResolveYul(InlineAssemblyAnnotation& _annotation, ReferencesResolver& _refRes):
			m_annotation(_annotation), m_outer(_refRes) {}

		void operator()(yul::FunctionDefinition const& _function) override
		{
			m_insideYulFunction = true;
			this->operator()(_function.body);
			m_insideYulFunction = false;
		}
		...
		{
			if (m_scope.find(_identifier.name) != m_scope.end())
				return;
			...
			auto declarations = m_outer.m_resolver.nameFromCurrentScope(_identifier.name.str());
			if (isSlot || isOffset)
			{
				...
				if (realName.empty())
				{
					m_outer.declarationError(_identifier.location, "In variable names _slot and _offset can only be used as a suffix.");
					return;
				}
				declarations = m_outer.m_resolver.nameFromCurrentScope(realName);
			}
			if (declarations.size() > 1)
			{
				m_outer.declarationError(_identifier.location, "Multiple matching identifiers. Resolving overloaded identifiers is not supported.");
				return;
			}
			else if (declarations.size() == 0)
				return;
			if (auto var = dynamic_cast<VariableDeclaration const*>(declarations.front()))
				if (var->isLocalVariable() && m_insideYulFunction)
				{
					m_outer.declarationError(_identifier.location, "Cannot access local Solidity variables from inside an inline assembly function.");
					return;
				}

			m_annotation.externalReferences[&_identifier].isSlot = isSlot;
			m_annotation.externalReferences[&_identifier].isOffset = isOffset;
			m_annotation.externalReferences[&_identifier].declaration = declarations.front();
		}
		...
		void operator()(yul::VariableDeclaration const& _varDecl) override
		{
			for (auto const& identifier: _varDecl.variables)
			{
				...
				if (isSlot || isOffset)
					m_outer.declarationError(identifier.location, "In variable declarations _slot and _offset can not be used as a suffix.");
				else if (
					auto declarations = m_outer.m_resolver.nameFromCurrentScope(namePrefix);
					!declarations.empty()
				)
				{
				...
				m_scope.emplace(identifier.name);
			}
	...
	private:
		bool m_insideYulFunction = false;
		InlineAssemblyAnnotation& m_annotation;
		ReferencesResolver& m_outer;
		set<yul::YulString> m_scope;
	} resolveYul(_inlineAssembly.annotation(), *this);

	resolveYul(_inlineAssembly.operations());
